...
def is_semantics_only_unordered_exact_match_post_ER(string_1, string_2, resolver):
    ...
    if resolved_tree_1 and tree_2:
        return resolved_tree_1.is_unordered_simi_match(tree_2) / tree_2.tree_num_nodes()
    return 0


def is_semantics_only_unordered_exact_match_post_ER_top_top(string_1, string_2, resolver, plot=False):
    '''
    Function to check if two strings have an unordered EM or not,
    without the non-semantic nodes. Entity resolution step will be performed on
    the semantics only tree representation for string_1. This function is only applicable for
    ...
    resolved_tree_1 = resolver.resolve_tree_into_TGT(tree_semantics_only_1)
    resolved_tree_2 = resolver.resolve_tree_into_TGT(tree_semantics_only_2)

    if plot:
        print(resolved_tree_1.pretty_string())
        print(resolved_tree_2.pretty_string())
    
    if resolved_tree_1 and resolved_tree_2:
        recall = resolved_tree_1.is_unordered_simi_match(resolved_tree_2) / resolved_tree_2.tree_num_nodes()
        precision = resolved_tree_1.is_unordered_simi_match(resolved_tree_2) / resolved_tree_1.tree_num_nodes()
